...
> ## 🟢 อัปเดตล่าสุด (2 ส.ค. 2026): v3 **สำเร็จแล้ว — ผ่านเกณฑ์คุณภาพ**
> โมเดลบน Hugging Face ตอนนี้คือ v3 (`checkpoint-200`) ซึ่ง **ดีกว่าโมเดลดั้งเดิมทั้งสองด้าน**:
> domain PPL **−15.7%** (8.830 → 7.442) และภาษาไทยทั่วไป **−12.6%** (8.034 → 7.024, ไม่มี catastrophic forgetting)
> รายละเอียดที่ [RESULTS.md §13](RESULTS.md)
>
> **บทเรียนจาก v2 ที่ล้มเหลว (+6.6% แย่ลง)** — สองสาเหตุที่แก้แล้ว:
> `learning_rate` สูงเกินไป (8e-5 → 2e-5) **และ** คลังข้อมูล 130MB ที่ใช้เทรนกลับเป็น Wikipedia ทั่วไป
> ที่ไม่เกี่ยวกับโดเมน >99% (เต่า, David Bowie, หนัง, วิดีโอเกม) → สร้างคลังใหม่เหลือ 606 เอกสารที่เกี่ยวข้องจริง
> ดู [RESULTS.md §11–12](RESULTS.md)
>
...
## ผลการรัน v3 (2 ส.ค. 2026) — 🟢 **สำเร็จ ผ่านเกณฑ์**

| ตัวชี้วัด | ก่อน CPT (base) | หลัง CPT (v3, step 200) | เปลี่ยนแปลง |
|---|---|---|---|
| **PPL โดเมนประสาทวิทยา** (40 บล็อก) | 8.830 | **7.442** | 🟢 **−15.7%** |
| **PPL ภาษาไทยทั่วไป** (88 บล็อก) | 8.034 | **7.024** | 🟢 **−12.6%** (ไม่ลืมของเดิม) |

`lr=2e-5 · lora_r=32/alpha=64 · cosine · effective batch 8,192 token/step · คลัง 1,746 บล็อก (~1.79M token)`

หยุดที่ step 200 เพราะ eval_loss เริ่มแย่ลงต่อเนื่องหลัง step 400 (ที่ step 1100 PPL พุ่งถึง 28.9)
พร้อม train loss ที่ลงถึง 0.31 — ลายเซ็นของการ**ท่องจำ** บนคลังที่เล็กเกินไปสำหรับ 15 epoch
จึงเผยแพร่ checkpoint ที่ดีที่สุดแทน ดู [RESULTS.md §13](RESULTS.md)

### ผลการรันเต็มสเกล v2 (1 ส.ค. 2026) — 🔴 ผลลบ ไม่ผ่านเกณฑ์

| ตัวชี้วัด | ก่อน CPT (base) | หลัง CPT (v2) | เปลี่ยนแปลง |
|---|---|---|---|
| **PPL โดเมนใหม่** | 9.858 | 10.507 | 🔴 **+6.6% (แย่ลง)** |
| PPL ภาษาไทยทั่วไป | 8.034 | ~~3.741~~ | ⚠️ ใช้ไม่ได้ — มี data leakage (แก้แล้ว ดู §11) |

สองสาเหตุ: `learning_rate=8e-5` สูงเกินไปทำให้เกิด "warmup shock" ตั้งแต่ต้น **และ** คลังข้อมูลที่ใช้เทรน
เป็น Wikipedia ทั่วไปที่ไม่เกี่ยวกับโดเมน >99% — วิเคราะห์ละเอียดพร้อมบั๊กที่พบระหว่างรัน
(เครื่องแครช 3 ครั้ง, data leakage, LR schedule reset) อยู่ที่ [RESULTS.md §11–12](RESULTS.md)
